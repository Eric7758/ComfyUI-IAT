...
def _build_prompt(tokenizer, messages):
    """Build chat prompt and disable reasoning traces when tokenizer supports it."""
    try:
...
def _resolve_model_path(model_path: str) -> str:
    """Resolve model_path against ComfyUI base path and plugin root.

    - Absolute paths are used as-is (after env/user expansion).
    - Relative paths are tried under ComfyUI base_path (if available), then under this plugin.
    """
    ...
    try:
        ...
        candidates.append(os.path.join(folder_paths.base_path, expanded))
    except Exception:
        pass

    plugin_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    candidates.append(os.path.join(plugin_root, expanded))
    ...
    return os.path.abspath(candidates[0] if candidates else expanded)
